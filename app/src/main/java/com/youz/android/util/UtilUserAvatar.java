...
public class UtilUserAvatar {
    ...
    public static int getAvatarRes(String userID, HashMap<String, Integer> hashMapAvatar) {
        if (hashMapAvatar.containsKey(userID)) {
            return hashMapAvatar.get(userID);
        } else {
            Random random = new Random();
            int index = random.nextInt(avatarArray.length);
            int res = avatarArray[index];
            hashMapAvatar.put(userID, res);
